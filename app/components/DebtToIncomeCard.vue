  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UBadge
          :color="getStatusColor(data.debtToIncome.status)"
          variant="subtle"
        >
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <p
          ...
          :class="getRatioColorClass(data.debtToIncome.ratio)"
        >
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <p class="text-lg font-semibold">
            {{ formatCurrency(data.debtToIncome.monthlyPayments) }}
          </p>
        </div>
        <div>
          <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">
          ...
          <p class="text-lg font-semibold">
            {{ formatCurrency(data.debtToIncome.monthlyIncome) }}
          </p>
        </div>
      </div>
...
const getGuidanceText = (ratio: number) => {
  if (ratio <= 20)
    return "✨ Excellent! You have plenty of financial flexibility.";
  if (ratio <= 36) return "👍 Good! Your debt is manageable.";
  if (ratio <= 43)
    return "⚠️ Consider reducing debt to improve financial health.";
