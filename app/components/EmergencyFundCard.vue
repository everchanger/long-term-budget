  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UBadge :color="getStatusColor(data.emergencyFund.status)" variant="subtle">
    ...
    <div class="space-y-4">
      <!-- Months Covered -->
      <div>
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">Months of Expenses Covered</p>
        <div class="flex items-baseline gap-2">
          <p class="text-3xl font-bold" :class="getMonthsColorClass(data.emergencyFund.monthsOfExpenses)">
            {{ data.emergencyFund.monthsOfExpenses.toFixed(1) }}
          </p>
          <p class="text-lg text-gray-500 dark:text-gray-400">/ {{ data.emergencyFund.targetMonths }}</p>
        </div>
      </div>
      ...
      <div class="space-y-2">
        <div class="relative h-4 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
          <div
            ...
            :style="{ width: `${Math.min((data.emergencyFund.monthsOfExpenses / data.emergencyFund.targetMonths) * 100, 100)}%` }"
          />
        </div>
      ...
      <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Current Savings Balance</p>
      ...
      <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
        <div class="flex items-start gap-2">
          <UIcon :name="getGuidanceIcon(data.emergencyFund.monthsOfExpenses)" class="w-5 h-5 mt-0.5" />
          <p class="text-sm text-gray-600 dark:text-gray-300">
            {{ getGuidanceText(data.emergencyFund.monthsOfExpenses, data.emergencyFund.targetMonths) }}
          </p>
        </div>
      </div>
    </div>
