...
describe('GET /api/households/:id/financial-health', () => {
  ...
  it('should return financial health data structure', async () => {
    const { data } = await $fetch<
      ApiSuccessResponse<{
        ...
        cashFlow: {
          ...
          annual: { income: number; expenses: number; debtPayments: number; netCashFlow: number }
          savingsRate: number
          status: string
        }
        debtToIncome: { ratio: number; monthlyPayments: number; monthlyIncome: number; status: string }
        emergencyFund: {
    ...
    expect(['excellent', 'good', 'fair', 'poor']).toContain(data.summary.overallHealth)
  })
