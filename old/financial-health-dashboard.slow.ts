...
test.describe("Financial Health Dashboard", () => {
  ...
  // Group related tests that can share page state
  test.describe.serial("Basic Navigation and Layout", () => {
    test("should display Financial Health link and navigate successfully", async ({
      page,
    }) => {
      // Check link is visible
      await expect(
        page.getByRole("link", { name: "Financial Health" })
      ).toBeVisible();
  ...
  test.describe.serial("Dashboard Cards and Content", () => {
    test.beforeAll(async ({ browser }) => {
      // Pre-navigate once for all tests in this group
    ...
    test("should display all cards and overall health summary", async ({ page }) => {
      await page.goto(`${BASE_URL}/financial-health`);
      ...
      await expect(
        page.getByText("Based on your net worth, cash flow")
      ).toBeVisible();
  ...
  test("should display cash flow and debt-to-income metrics", async ({ page }) => {
      await page.goto(`${BASE_URL}/financial-health`);
    ...
    const hasGuidance = await page
      .getByText(new RegExp(guidanceTexts.join("|")))
      .isVisible();
  ...
  test("should show excellent health status with strong financials", async ({
    page,
  }) => {
  ...
  test("should show poor health status with challenging financials", async ({
    page,
  }) => {
    ...
    const recommendations = page
      .locator("text=Recommended Actions")
      .locator("..");
    await expect(
      recommendations.getByText(/Reduce Debt|Emergency Fund|Savings/)
    ).toBeVisible();
  });

  test("should show good/fair health with moderate financials", async ({
    page,
  }) => {
    ...
    const statusText = await page
      .locator("text=/Good|Fair|Excellent/")
      .first()
      .textContent();
  ...
  test("should be responsive on mobile viewport", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Debt-to-Income Ratio" })
    ).toBeVisible();
    await expect(
