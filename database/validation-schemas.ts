...
export const insertHouseholdSchema = createInsertSchema(households, {
  name: z.string().min(1, 'Household name is required').min(2, 'Name must be at least 2 characters'),
  userId: z.number().positive('User ID is required'),
})

export const selectHouseholdSchema = createSelectSchema(households)

export const updateHouseholdSchema = insertHouseholdSchema.partial().omit({ id: true, createdAt: true, userId: true })

// Person schemas
export const insertPersonSchema = createInsertSchema(persons, {
  name: z.string().min(1, 'Name is required').min(2, 'Name must be at least 2 characters'),
  age: z.number().min(0, 'Age must be positive').max(150, 'Age must be realistic').optional(),
  householdId: z.number().positive('Household ID is required'),
})

export const selectPersonSchema = createSelectSchema(persons)

export const updatePersonSchema = insertPersonSchema.partial().omit({ id: true, createdAt: true, householdId: true })
...
export const updateIncomeSourceSchema = insertIncomeSourceSchema.partial().omit({ id: true, createdAt: true, personId: true })
...
export const updateExpenseSchema = insertExpenseSchema.partial().omit({ id: true, createdAt: true, personId: true })

// Savings Account schemas
export const insertSavingsAccountSchema = createInsertSchema(savingsAccounts, {
  name: z.string().min(1, 'Account name is required'),
  balance: z.number().min(0, 'Balance must be positive'),
  interestRate: z.number().min(0, 'Interest rate must be positive').max(100, 'Interest rate cannot exceed 100%').optional(),
  personId: z.number().positive('Person ID is required'),
})

export const selectSavingsAccountSchema = createSelectSchema(savingsAccounts)

export const updateSavingsAccountSchema = insertSavingsAccountSchema.partial().omit({ id: true, createdAt: true, personId: true })
...
export const insertLoanSchema = createInsertSchema(loans, {
  ...
  monthlyPayment: z.number().min(0, 'Monthly payment must be positive').optional(),
  personId: z.number().positive('Person ID is required'),
})

export const selectLoanSchema = createSelectSchema(loans)

export const updateLoanSchema = insertLoanSchema.partial().omit({ id: true, createdAt: true, personId: true })
...
export const updateScenarioSchema = insertScenarioSchema.partial().omit({ id: true, createdAt: true, householdId: true })
